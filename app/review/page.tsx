...
export default async function ReviewPage() {
  ...
  return (
    <main className="min-h-dvh px-6 py-8 pb-24">
      <h1 className="mb-4 text-xl font-semibold text-neutral-900">{t(locale, "navReview")}</h1>

      {items.length === 0 ? (
        <p className="px-2 py-8 text-center text-sm text-neutral-500">{t(locale, "reviewEmpty")}</p>
      ) : (
