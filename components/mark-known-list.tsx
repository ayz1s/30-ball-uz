...
export function MarkKnownList({ topics, locale }: { topics: MarkKnownTopic[]; locale: Locale }) {
  ...
  if (topics.length === 0) {
    return <p className="px-2 py-8 text-center text-sm text-neutral-500">{t(locale, "markKnownEmpty")}</p>;
  }
    ...
    <div className="space-y-2">
      {topics.map((topic) => {
        ...
        return (
          <div
            key={topic.id}
            className="flex min-h-14 items-center justify-between gap-3 rounded-xl border border-neutral-200 px-4 py-3"
          >
            <div>
              <p className="text-neutral-900">{topic.title}</p>
            ...
            <button
              ...
              className={`min-h-11 shrink-0 rounded-lg px-3 py-2 text-sm font-medium ${
                known ? "bg-green-100 text-green-800" : "bg-blue-50 text-blue-700"
              }`}
            >
