...
11. Теория должна быть полноценной, а не куцым определением на два предложения. Если тема вводит именованное правило, состоящее из нескольких шагов или случаев (например порядок действий, знаки при умножении, свойства пропорции) — разложи его в тексте или в отдельном rule-блоке как пронумерованный или чётко разделённый список всех случаев, а не одной обтекаемой фразой. Количество steps-блоков с полностью разобранными примерами должно быть таким, чтобы явно показать РАЗНЫЕ вариации техники (например: без скобок / со скобками / с отрицательными числами / с дробями) — не ограничивайся одним-двумя однотипными примерами, если у техники есть несколько существенно разных случаев применения.
